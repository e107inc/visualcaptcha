	...
	*/
	...
	function visualcaptcha_options()
	{
		...
		else
		{
			...
			if(isset($_POST['visualcaptcha_nonce_opt']) && check_admin_referer('visualcaptcha_addpot', 'visualcaptcha_nonce_opt'))
			{
				...
				if(empty($hook_name) || empty($hook_action) || empty($hook_filter))
				{
					if(empty($hook_name))
					{
						$new_hook_error['visualcaptcha_add_opt_name'] = 'error';
					}
					if(empty($hook_action))
					{
						$new_hook_error['visualcaptcha_add_opt_action'] = 'error';
					}
					if(empty($hook_filter))
					{
						$new_hook_error['visualcaptcha_add_opt_filter'] = 'error';
					}
				}
		...
		<div class="wrap">
			...
			<div>
				<div class="visualcaptcha_promo_img"><a href="http://visualcaptcha.net" target="_blank"><img
							src="<?php echo WP_CONTENT_URL, '/plugins/visualcaptcha/application.png'; ?>"
							alt="visualCaptcha on multiple devices"/></a></div>
				<p><strong>visualCaptcha</strong> is a configurable captcha solution, focusing on <strong>accessibility
						&amp; simplicity</strong> whilst maintaining <strong>security</strong>.</p>

				<p>More information over at <a href="http://visualcaptcha.net" target="_blank">visualcaptcha.net</a>.
				</p>
			...
			<form name="form" method="post" action="">
				<h3>
					<small><?php _e('Enable visualCaptcha in the following places:', 'visualcaptcha'); ?></small>
				</h3>
				<table class="form-table">
					<?php foreach($visualcaptcha_current_hooks as $name => $hook)
					{
						...
						<tr valign="top">
							...
							<td>
								<input name="visualcaptcha_form_hooks[<?php echo $name ?>][checked]" value="0"
								       type="hidden"/>
								<label style="margin-right: 5px">
									<input name="visualcaptcha_form_hooks[<?php echo $name ?>][checked]" value="1"
									       type="checkbox" <?php if($hook['checked']) echo 'checked="checked"' ?> />
									Enabled
								</label>
								with
								<select name="visualcaptcha_form_hooks[<?php echo $name ?>][images]">
									<option
										value="4" <?php echo (isset($hook['images']) && !empty($hook['images']) && $hook['images'] == 4) ? 'selected="selected"' : '' ?> >
										4
									</option>
									<option
										value="5" <?php echo (isset($hook['images']) && !empty($hook['images']) && $hook['images'] == 5) ? 'selected="selected"' : '' ?> >
										5
									</option>
									<option
										value="6" <?php echo (isset($hook['images']) && !empty($hook['images']) && $hook['images'] == 6) ? 'selected="selected"' : '' ?> >
										6
									</option>
								</select>
								images
			...
			<form method="post" action="admin.php?page=<?php echo $_GET['page'] ?>">
				<table class="form-table">
					<tr valign="top">
						<td>
							<label
								class="<?php echo (isset($new_hook_error['visualcaptcha_add_opt_name'])) ? 'visualcaptcha_error' : ''; ?>"><?php _e('Option Name', 'visualcaptcha') ?>
								*</label><br/>
							<input name="visualcaptcha_add_opt_name" class="visualcatpcha_form_text_input"
							       value="<?php echo $new_hook['name'] ?>"/>
							<small>(<?php _e('Name for the "Add visualCaptcha to:" List', 'visualcaptcha') ?>)</small>
							<br/>
							<label
								class="<?php echo (isset($new_hook_error['visualcaptcha_add_opt_action'])) ? 'visualcaptcha_error' : ''; ?>"><?php _e('Display Hook', 'visualcaptcha') ?>
								*</label><br/>
							<input name="visualcaptcha_add_opt_action_hook" class="visualcatpcha_form_text_input"
							       value="<?php echo $new_hook['action'] ?>"/>
							<small>
								(<?php _e('Action Hook used to show visualCaptcha - i.e. "login_form"', 'visualcaptcha') ?>
								)
							</small>
							<br/>
							<label
								class="<?php echo (isset($new_hook_error['visualcaptcha_add_opt_filter'])) ? 'visualcaptcha_error' : ''; ?>"><?php _e('Validation Hook', 'visualcaptcha') ?>
								*</label><br/>
							<input name="visualcaptcha_add_opt_filter_hook" class="visualcatpcha_form_text_input"
							       value="<?php echo $new_hook['filter'] ?>"/>
							<small>
								(<?php _e('Filter Hook used to validate a visualCaptcha submission - i.e. "authenticate"', 'visualcaptcha') ?>
								)
							</small>
							<br/>
							<small><a href="http://adambrown.info/p/wp_hooks/version/"
							          target="_new"><?php _e('DEFAULT HOOKS REFERENCE', 'visualcaptcha') ?></a></small>
							<br/><br/>
							<label
								class="<?php echo (isset($new_hook_error['visualcaptcha_terms'])) ? 'visualcaptcha_error' : ''; ?>"><input
									type="checkbox" name="visualcaptcha_terms"
									value="1"/> <?php _e('By using this option you confirm that you have the proper knowledge to use Wordpress Actions and Filters. If you don\'t, please don\'t use it') ?>
								*</label><br/>
